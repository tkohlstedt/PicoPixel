...
void default_mcu_reset(void) {watchdog_enable(1, 1);
								while(1);}
...
int sendsuccess = 1;
	if(send_len){
		int sendret = send(s, buf, send_len);
		if ( sendret != send_len)
		{
			sendsuccess = 0;
			flag_datasend_end = 0;
		printf("Send failed %i return %i\n\r",send_len,sendret);
		}
	}
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			if(p_http_request->TYPE == PTYPE_CGI)
			{
				...
				{
					content_addr = (uint32_t)pHTTP_TX;
					HTTPSock_Status[get_seqnum].storage_type = CODEFLASH;
		...
		case METHOD_POST :
			...
			if(p_http_request->TYPE == PTYPE_CGI)	// HTTP POST Method; CGI Process
			{
				...
				if(content_found) // && (file_len <= (DATA_BUF_SIZE-(strlen(RES_CGIHEAD_OK)+8))))
				{
					...
					// need to relocate the reset.  This will reset before sending complete response
					if(content_found == HTTP_RESET) 
					{
#ifdef _HTTPSERVER_DEBUG_
				printf("> HTTPSERVER restarting \n\r");
#endif
						HTTPServer_ReStart();
					}
				}
